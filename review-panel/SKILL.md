...
The diversity invariant (Codex + Gemini + Opus, not one family) is load-bearing — keep all three families. The Gemini seat is a Google-family model served through `agy`; do not swap it for agy's Claude/GPT-OSS models, which would collapse the invariant.
...
- Gemini-via-agy can error on auth/quota (`RESOURCE_EXHAUSTED`, `IneligibleTier`, `UNSUPPORTED_CLIENT`). For high-stakes waves, do not ship without the third family: retry, switch tier via `GEMINI_MODEL` (Flash↔Pro), or substitute a second Opus pass labelled "standing in for Gemini," and note the gap. (The watchdog also self-heals any stale gemini-cli `GEMINI_MODEL` id by remapping it to the agy default.)
